...
class FlagGame(BoxLayout):
    ...
    def setup_game(self):
        ...
        self.score_label = Label(text=f"Score: {self.score}", size_hint=(1,0.1))
        self.timer_label = Label(text=f"Waktu: {self.time_left}", size_hint=(1,0.1))

        self.add_widget(self.score_label)
        self.add_widget(self.timer_label)

        self.flag_image = AsyncImage(size_hint=(1,0.5))
    ...
    def check_answer(self, instance):
        if instance.text == self.country:
            ...
            self.show_effect(instance, True)
        else:
            self.show_effect(instance, False)
    ...
    def shake_button(self, button):
        anim = Animation(x=button.x-10, duration=0.05) + Animation(x=button.x+10, duration=0.05)
    ...
    def show_congratulations(self):
        ...
        msg = "Keren juga lu tod!" if self.score == self.max_rounds else "Yahahah Bego!"

        self.add_widget(Label(text=msg, font_size=30))
